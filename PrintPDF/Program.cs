...
namespace PrintPDF
{
    class Program
    {
        ...
        private static void startPrint()
        {
            ...
            try
            {
                currentDirectory = Directory.GetCurrentDirectory(); //Ruta donden se encuntra el programa
                FileToRead = currentDirectory + @"\PrintingDetails.json"; //Archivo JSON con detalles de impresion
            ...
            {
                Console.Clear();
                Console.WriteLine("Algo salió mal: " + e.Message);
            ...
            List<PrintConfigModel> configsPrint = new List<PrintConfigModel>();


            //verificar si son varios o solo un pdf
            // si el json empiexa con { es solo uno
            //si el json empieza con [ son varios
            ...
            switch (text[0])
            {
                case '{':
                    ...
                    //Serializar objeto 
                    PrintConfigModel configPrint = JsonConvert.DeserializeObject<PrintConfigModel>(text);
                    configsPrint.Add(configPrint);
                    break;

                case '[':

                    configsPrint = JsonConvert.DeserializeObject<List<PrintConfigModel>>(text);

                    break;
                default:
                    Console.Clear();
                    Console.WriteLine("Algo salió mal: El archivo de configuraciones de impresion (json) no tiene contenido.");
                    Console.WriteLine("Presione una tecla para Finalizar");
                    Console.ReadKey();
                    return;
            ...
            foreach (var configPrint in configsPrint)
            {
                //Ruta y nomre del archivo
                int pagesPdf = 1;

                string PDFtoPrint = currentDirectory + @"\" + configPrint.report_name;
                ...
                using (var progress = new ProgressBar())
                {
                    ...
                    using (WebClient webClient = new WebClient())
                    {
                        try
                        {
                            webClient.DownloadFile(configPrint.url_report, PDFtoPrint);
                        }
                        catch (Exception ex)
                        {
                            Console.Clear();
                 ...
                 */
                for (int i = 0; i < configPrint.number_prints; i++)
                {
                    //Formato para el dialog de impresion 
                    string info_print = $"{pagesPdf}{Environment.NewLine} Copia {i + 1} de {configPrint.number_prints}{Environment.NewLine}{Environment.NewLine}{configPrint.document_name}";
                    ...
                    using (PdfDocument doc = new PdfDocument())
                    {
                        ...
                        doc.PrintSettings.PrinterName = configPrint.printer_name;
